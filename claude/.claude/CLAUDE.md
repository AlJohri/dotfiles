...
- **spr stacks** (`spacedentist/spr`): for any spr operation — `spr diff`/`--all`, editing a published stack, restacking, landing, reconciling after a merge — **use the `spr` skill** (full workflow + mechanics live there). Always pass `--draft` to `spr diff` (limits blast radius). Two irreversible guardrails to keep in mind even before that skill loads:
