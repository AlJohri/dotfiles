...
- **Reviewing diffs in an editor:** a global `git dt` alias = `difftool -d --no-symlinks` opens all changed files in one Zed multi-diff window (Zed is configured as `diff.tool`). Use `git dt` (working tree vs HEAD), `git dt main` (whole branch vs a ref), or `git dt <commit>`. The `--no-symlinks` is load-bearing and is baked into the alias: in `-d` (dir-diff) mode git symlinks the working-tree side back to the real files, and Zed (like most editors) won't dereference those symlinks, so every file renders as *deleted*. There is no config key to default this (no `difftool.symlinks`), which is why it lives in the alias — don't strip it. Plain `git diff` / `git show` / `git log -p` are unaffected (still inline CLI).
