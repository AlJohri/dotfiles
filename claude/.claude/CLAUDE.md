...
- **spr stacks** (`spacedentist/spr`): for any spr operation — `spr diff`/`--all`, editing a published stack, restacking, landing, reconciling after a merge — **use the `spr` skill** (full workflow + mechanics live there). Two irreversible guardrails to keep in mind even before that skill loads:
