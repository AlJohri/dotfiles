...
# Stow the dotfiles, then review every file that collided with a pre-existing
# (omarchy/system) file and now differs from our tracked version.
#
# This only does anything on a fresh install or the first run after an
# `omarchy upgrade`. On a steady-state machine everything is already symlinked,
# so there is nothing to adopt and this is a silent no-op.
#
# How it works: `make stow-<target>` uses `stow --adopt`, which pulls any
# pre-existing real file at a target path INTO the repo so the symlink can be
# created -- meaning the system's defaults temporarily overwrite our tracked
# files. Files unique to us (or byte-identical to the system's) never show up
# here; only genuine drift does. For each drifted file you choose:
...
# We use `git checkout`/`git add` to resolve each file, which would clobber any
# unrelated uncommitted work. Require a clean tree first.
